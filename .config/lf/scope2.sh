...
handle_extension() {
    case "${FILE_EXTENSION_LOWER}" in
        ...
        pdf)
            ...
            pdftotext -l 10 -nopgbrk -q -- "${FILE_PATH}" - | fmt && exit 5
            mutool draw -F txt -i -- "${FILE_PATH}" 1-10 | fmt && exit 5
...
handle_mime() {
    ...
    case "${mimetype}" in
        ...
        image/*)
            # Preview as text conversion
            # img2txt --gamma=0.6 -- "${FILE_PATH}" && exit 4
            exiftool "${FILE_PATH}" && exit 5
	    # catimg -w 100 "${FILE_PATH}" && exit 5
            exit 1;;
